...
public class MainPhaseMessageConsumer implements Runnable {
    ...
    public MainPhaseMessageConsumer(MessageQueue messageQueue, WebSocketConnectionPool connectionPool,
                                    MessageTimer messageTimer, MetricsCollector metricsCollector,
                                    AtomicInteger messagesSent, int totalMessages) {
    ...
    private void sendMessageWithRetry(MessageQueueEntry entry) {
        ...
        for (int attempt = 1; attempt <= Constants.MESSAGE_RETRY_ATTEMPTS; attempt++) {
            try {
                ...
                // Record send timestamp for timeout tracking and metrics
                messageTimer.recordMessageSent(entry.getMessage(), entry.getRoomId());
                metricsCollector.recordMessageSent(entry.getMessage(), entry.getRoomId());
