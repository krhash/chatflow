 ...
 * 2. Check if message was sent by this client
 * 3. Detect if message is an ACK echo (marks original message complete)
 * 4. Queue ACKs for original messages
 *
 * 100 processor threads work in parallel processing the shared receiver queue.
 */
public class MessageProcessorWorker implements Runnable {
    ...
    /**
     * Process message: determine if it's an original message or ACK echo.
     */
    private void processMessage(ChatMessage message) {
        try {
            String messageId = message.getMessageId();

            // Atomic remove - only one thread will succeed
            if (sentMessageIds.remove(messageId)) {
                ...
                if (originalMessageId != null) {
                    // This is an ACK message echoed back - original message complete!
                    messagesCompleted.incrementAndGet();
                    logger.debug("Message {} completed (ACK {} received)", originalMessageId, messageId);
                } else {
                    // This is an original message (JOIN/TEXT/LEAVE) - queue ACK
                    queueAck(message);
                }
            }
            // If remove returns false, another thread already processed this message
